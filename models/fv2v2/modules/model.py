...
class ExpTransformerTrainer(GeneratorFullModelWithSeg):
    ...
    def forward(self, x):
        ...
        if self.stage == 2:
            ...
            cycled_source = self.cyce(x['source'], bs, 1)
            cycled_kp_source = self.cycle(kp_source, bs, 1)
            cycled_tf_output = self.exp_transformer(cycled_kp_source, kp_driving['value'])
            cycled_src_exp = cycled_tf_output['src_exp']
            drv_exp = cycled_tf_output['drv_exp']
            cycled_kp_source['exp'] = src_exp
            kp_driving['exp'] = drv_exp
            
            cycled_generated = self.generator(cycled_source, kp_source=cycled_kp_source, kp_driving=kp_driving)
            cycled_output = cycled_generated['prediction']
            cycled_output_kp = 
