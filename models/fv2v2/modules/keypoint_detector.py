...
class ExpTransformer(nn.Module):
    ...
    def __init__(self, block_expansion, feature_channel, input_dim, num_kp, image_channel, max_features, num_bins=66, num_layer=1, num_heads=32, code_dim=8, latent_dim=256, estimate_jacobian=True, sections=None):
        ...
        # self.id_encoder = ResnetEncoder(num_kp=self.num_kp, latent_dim=latent_dim)
        
        self.kp_encoder = LinearEncoder(input_dim=2048, latent_dim=self.latent_dim, depth=0)
        ...
        self.delta_style_extractor_from_mesh = LinearEncoder(input_dim=2048, latent_dim=self.latent_dim, output_dim=self.latent_dim // 2, depth=2)
    ...
    def encode(self, x, placeholder=['kp', 'delta']):
        ...
        if 'kp' in placeholder:
            output['kp'] = self.kp_encoder(id_embedding)
            
        if 'delta' in placeholder:
            style_from_mesh = self.delta_style_extractor_from_mesh(id_embedding)
            exp_from_mesh = self.delta_exp_extractor_from_mesh(id_embedding)
