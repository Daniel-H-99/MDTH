...
def get_src_drv_path(src_dir, drv_dir):
    ...
    src_frame_path = src_frame_path.replace('%d', '%_d')
    video_path = video_path.replace('%d', '%_d')
...
def make_concat_video_cmd(output_name, src_path, drv_path, result_path_list):
    ...
    concat_src_list = [drv_path, src_path] + result_path_list
...
def main():
    ...
    np.savetxt(list_file_path, video_list, fmt="%s", comments=None)

    for i, (vid, label) in tqdm(enumerate(zip(video_list, labels))):
        ...
        output_name = os.path.join(output_video_dir, '{:02d}_'.format(i) + vid + '.mp4')
