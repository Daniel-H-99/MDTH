...
def make_animation(source_image, driving_video, exp_transformer, generator, kp_detector, he_estimator, relative=True, adapt_movement_scale=True, estimate_jacobian=True, cpu=False, free_view=False, yaw=0, pitch=0, roll=0):
    with torch.no_grad():
        predictions = []
        ...
        yaw_src, pitch_src, roll_src, t_src, exp_src = torch.tensor(he_source['yaw']), torch.tensor(he_source['pitch']), torch.tensor(he_source['roll']), torch.tensor(he_source['t']), torch.tensor(he_source['exp'])
        he_driving_initial = he_estimator(driving[:, :, 0])

        kp_source = keypoint_transformation(kp_canonical, he_source, estimate_jacobian)
        he_source['yaw'] = torch.tensor(he_driving_initial['yaw'])
        he_source['pitch'] = torch.tensor(he_driving_initial['pitch'])
        he_source['roll'] = torch.tensor(he_driving_initial['roll'])
        he_source['t'] = torch.tensor(he_driving_initial['t'])
        he_source['exp'] = torch.tensor(exp_src)

        kp_source_initial = keypoint_transformation(kp_canonical, he_source, estimate_jacobian)
        ...
        for frame_idx in tqdm(range(driving.shape[2])):
            ...
            # he_source['yaw'] = torch.tensor(he_driving['yaw'])
            # he_source['pitch'] = torch.tensor(he_driving['pitch'])
            # he_source['roll'] = torch.tensor(he_driving['roll'])
            # he_source['t'] = torch.tensor(he_driving['t'])
            # he_source['exp'] = torch.zeros_like(torch.tensor(exp_src))

            ## fix head pose
            # he_driving['yaw'] = torch.tensor(yaw_src)
            # he_driving['pitch'] = torch.tensor(pitch_src)
            # he_driving['roll'] = torch.tensor(roll_src)
            # he_driving['t'] = torch.tensor(t_src)
            # he_driving['exp'] = torch.tensor(exp_src)
            ...
            # kp_source_tmp = keypoint_transformation(kp_canonical, he_source, estimate_jacobian)

            kp_norm = kp_source_initial
            # kp_norm = kp_driving
            # kp_norm = normalize_kp(kp_source=kp_source_initial, kp_driving=kp_driving,
...
if __name__ == "__main__":
    ...
    exp_transformer, generator, kp_detector, he_estimator = load_checkpoints(config_path=opt.config, checkpoint_path=opt.checkpoint, checkpoint_ref_path=opt.checkpoint_ref_path, gen=opt.gen, cpu=opt.cpu)
    ...
    if opt.find_best_frame or opt.best_frame is not None:
        ...
        predictions_forward = make_animation(source_image, driving_forward, exp_transformer, generator, kp_detector, he_estimator, relative=opt.relative, adapt_movement_scale=opt.adapt_scale, estimate_jacobian=estimate_jacobian, cpu=opt.cpu, free_view=opt.free_view, yaw=opt.yaw, pitch=opt.pitch, roll=opt.roll)
        predictions_backward = make_animation(source_image, driving_backward, exp_transformer, generator, kp_detector, he_estimator, relative=opt.relative, adapt_movement_scale=opt.adapt_scale, estimate_jacobian=estimate_jacobian, cpu=opt.cpu, free_view=opt.free_view, yaw=opt.yaw, pitch=opt.pitch, roll=opt.roll)
        predictions = predictions_backward[::-1] + predictions_forward[1:]
    else:
        predictions = make_animation(source_image, driving_video, exp_transformer, generator, kp_detector, he_estimator, relative=opt.relative, adapt_movement_scale=opt.adapt_scale, estimate_jacobian=estimate_jacobian, cpu=opt.cpu, free_view=opt.free_view, yaw=opt.yaw, pitch=opt.pitch, roll=opt.roll)
