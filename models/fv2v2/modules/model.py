...
class GeneratorFullModelWithSeg(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        if self.loss_weights['motion_match'] != 0:
            ...
            it_section_eye = it_section[:,  x['source_mesh']['OPENFACE_EYE_IDX'][0].long()]
            motion_GT_eye = motion_GT[:,  x['source_mesh']['OPENFACE_EYE_IDX'][0].long()]
            it_section_mouth = it_section[:,  x['source_mesh']['OPENFACE_LIP_IDX'][0].long()]
            motion_GT_mouth = motion_GT[:,  x['source_mesh']['OPENFACE_LIP_IDX'][0].long()]
    ...
    def forward(self, x, cycled_drive=False):
        if self.stage == 1:
            ...
            if self.loss_weights['guide'] != 0:
                ...
                delta = driving_mesh['delta'].flatten(1)
                assert delta_GT.shape == delta.shape
                loss_values['guide'] = self.loss_weights['guide'] * F.mse_loss(delta, delta_GT)
