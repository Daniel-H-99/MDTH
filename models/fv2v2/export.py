...
def adapt_values(origin, values, minimum=None, maximum=None, rel_minimum=None, rel_maximum=None, scale=None, center_align=False, center=None, clip=False):
    # origin: float
    ...
    if clip:
        adapted_values = adapted_values.clip(minimum, maximum)
...
def filter_mesh(meshes, source_mesh, SCALE):
    ...
    tmp_views[:, :3, 3] = t_filtered.astype(np.float32)
    
    # print(f'tmp views shape: {tmp_views.transpose(0, 1, 2).shape}')
    ...
    t_xs_adapted = adapt_values(t_x_source, t_xs, minimum=32, maximum=224, center_align=False)
    t_ys_adapted = adapt_values(t_y_source, t_ys, minimum=32, maximum=224, center_align=False)
    t_zs_adapted = adapt_values(t_z_source, t_zs, center_align=False)
    ...
    for R_x, R_y, R_z, t_x, t_y, t_z, view_t_x, view_t_y, view_t_z, project, mesh in zip(R_xs_filtered, R_ys_filtered, R_zs_filtered, t_xs_filtered, t_ys_filtered, t_zs_filtered, view_t_xs, view_t_ys, view_t_zs, projects, meshes):
        ...
        rot_src[:3, 3] = new_view_t
        # rot_src[:3, 3] = new_t.numpy().astype(np.float32)
...
def test_model_with_exp(opt, generator, exp_transformer, kp_extractor, he_estimator, gpu_list):
    ...
    source_mesh['t'] = pose_p['U'][3, :3].copy()
    ...
    # source_mesh['he_t'][2] = -source_mesh['he_t'][2]
    ...
    while True:
        continue
    b = np.linalg.inv(source_mesh['U'][:3, :3]) @ tmp
    source_mesh['b'] = b
   ...
   ## iris_movement adaption
    ...
    for i, driving_landmark in enumerate(driving_landmarks_from_flame):
        ...
        if relative_headpose:
            ...
            print(f'driven_pose_index: {driven_pose_index}')
            ...
            print(f'viewport: {mesh["viewport"]}')
            print(f'proj: {mesh["proj"]}')
            print(f'viewj: {mesh["view"]}')
        else:
