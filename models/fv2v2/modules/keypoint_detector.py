...
class ExpTransformer(nn.Module):
    ...
    def __init__(self, block_expansion, feature_channel, input_dim, num_kp, image_channel, max_features, num_bins=66, num_layer=1, num_heads=32, code_dim=8, latent_dim=256, estimate_jacobian=True, sections=None):
        ...
        self.delta_exp_extractor_from_mesh_fc = LinearEncoder(input_dim=2048, latent_dim=self.latent_dim, output_dim=self.num_heads, depth=2)
    ...
    def encode(self, x, placeholder=['kp', 'delta']):
        ...
        if 'delta' in placeholder:
            ...
            exp_from_mesh = self.delta_exp_extractor_from_mesh_fc(self.delta_exp_extractor_from_mesh(x['img']))
    ...
    def decode(self, embedding):
        ...
        if 'delta_style_code' in embedding and 'delta_exp_code' in embedding:
            ...
            x = self.delta_exp_code_decoder(x)
