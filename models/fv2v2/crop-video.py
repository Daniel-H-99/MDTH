...
def process_frame(inp, output, fa):
    frame = imageio.imread(inp)
    bbox = extract_bbox(frame, fa)[0]
    compute_bbox_image(bbox, frame.shape, inp, output, increase_area=0.1)

def compute_bbox_image(bbox, frame_shape, inp, output, increase_area=0.1):
    ...
    # width_increase = max(increase_area, 3 * increase_area)
    # height_increase = max(increase_area, 3 * increase_area )
    print(f'width; {width}')
    ...
    scale_file_path = output[:-4] + '.txt'
    np.savetxt(scale_file_path, np.array([h, w]), delimiter=',', comments=None)
    cmd = f'ffmpeg -y -i {inp} -filter:v "fps=25, crop={w}:{h}:{left}:{top}, scale={scale}" {output}'
    print(f'cmd: {cmd}')
    os.system(cmd)
    return 
...
def compute_bbox_trajectories(trajectories, fps, frame_shape, args):
    ...
    for i, (bbox, tube_bbox, start, end) in enumerate(trajectories):
        if (end - start) > args.min_frames:
            command = compute_bbox(start, end, fps, tube_bbox, frame_shape, inp=args.inp, image_shape=args.image_shape, increase_area=args.increase, output=args.output)
...
def process_video(args):
    ...
    if 'fps' not in video.get_meta_data():
        ...
        process_frame(args.inp, args.output, fa)
        return []
    else:
        trajectories = []
        previous_frame = None
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output", default="crop.mp4", help='output name')
